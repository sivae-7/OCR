...
image_path = './images/2024-FC-EROLLGEN-S22-175-FinalRoll-Revision2-ENG-185-WI-17.png'
...
# sections = re.split(r'\n(?=\d{1,4}[\)\.,\s]?\s*(MML|RRN|RRRN))', cleaned_text)
...
def parse_voter_section(section):
    ...
    relation_names = [rel.strip() for rel in lines[2].replace("Father Name", "").replace("Husband Name", "").replace("Mother Name", "").split("  ") if rel.strip()]
